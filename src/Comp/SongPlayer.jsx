  ...
  useEffect(() => {
    ...
    const handlePlay = () => {
        if (songOn) {
          ...
          console.log(prevSong+"from handlePlay");
        }
         else
          {
          audioRef.current?.pause();
        }
        setSongOn(!songOn); // only toggle once, at the end
      };
      const handlePrev = () => {
        if (prevSong !== null) {
            setNum(prevSong);
            setSongOn(true); // Ensure play is triggered again
          console.log("prev" + prevSong);
        }
      };
    const handleNext = ()=>{
         console.log(prevSong+"from handleNext 1");
        ...
        if (num !== randomNumber) {
            ...
            console.log(prevSong+"from handleNext 2");
            setSongOn(true);
        }
        else{
            handleNext();  
    ...
    const currentSong = songs[num] || {}; // Fallback to empty object if undefined
    const { id, title, fileUrl } = currentSong; // Now safe to destructure
    const formatTime = (time) => {
        const minutes = Math.floor(time / 60);
        const seconds = Math.floor(time % 60);
        return `${minutes}:${seconds < 10 ? '0' : ''}${seconds}`;
      };
    if (!songs.length) return <p className="text-center text-white">Loading...</p>;
    const handleVolumeChange = (e) => {
        audioRef.current.volume = e.target.value;
    };

    return (
    <div className={`bg-[#001311] lg:max-w-sm  p-6 rounded-2xl shadow-lg ${mode?"bg-[#001311]":"bg-white border border-[#132523]"} rounded-3xl p-2 border-transparent shadow-lg`}>
        <div className=" mb-14 mt-5">
            <img className="w-2/3 lg:w-1/2 mx-auto rounded-3xl" src="https://i.pinimg.com/474x/99/98/c6/9998c6fb53282508aba3353f81b68a88.jpg" alt="" />
        </div>
      ...
      <div className="">
        ...
        className={`w-full h-1.5 ${mode?"bg-gray-700":"bg-gray-300"} rounded-full appearance-none 
...
{fileUrl && (
  ...
  onLoadedMetadata={() => {
    setDuration(audioRef.current.duration); // Also track total duration
  }}
/>
)}
    </div>
